"""
First implementation of the optimization.
...
3. Apply Sinc Comb Filter to extract fetal signal using a known frequency (2Hz and 4Hz)
4. Compute th e Energy Ratio Metric between the filtered and original signal
5. Optimize the window parameters to maximize the metric 
"""
import numpy as np
import torch
import torch.nn as nn
import torch.optim as optim
from joint_tof_opt.compact_stat_process import NthOrderMoment
...
comb_filter = CombSeparator(
    fs=sampling_rate,
    f0=fetal_f,
    f1=2 * fetal_f,
    half_width=0.3,
    filter_length=len(time_axis) // 4,  # TODO: Choose appropriate filter length later
)
...
for epoch in range(num_epochs):
    ...
    filtered_signal = comb_filter(compact_stats_reshaped)

    # Step 3: Compute energy ratio metric
    metric_value = energy_ratio_metric(filtered_signal, compact_stats)
