...
class TestComputeTofDiscrete:
    ...
    def test_basic_histogram(self):
        ...
        histogram, _, __ = compute_tof_discrete(
    ...
    def test_all_photons_same_time(self):
        ...
        histogram, _, __ = compute_tof_discrete(partialpath_table, light_speed, tissue_model, num_bins=5)
...
class TestComputeTofDataSingleTimePoint:
    ...
    def test_basic_tof_data(self):
        ...
        tof_data = compute_tof_data_single_time_point(partialpath_table, light_speed, tissue_model, num_bins=10)

        assert tof_data.tof_series.shape == (1, 10)
        assert tof_data.var_series.shape == (1, 10)
        assert tof_data.bin_edges.shape == (11,)
        assert tof_data.bin_centers.shape == (10,)
        assert tof_data.meta_data is None

    def test_data_match(self):
        """Test that ToFData contents match expected histogram and variance."""
        partialpath_table = np.array(
            [
                [100.0],
                [200.0],
        ...
        expected_histogram, expected_bin_edges, expected_var = compute_tof_discrete(
        ...
        np.testing.assert_array_almost_equal(tof_data.tof_series.flatten(), expected_histogram)
