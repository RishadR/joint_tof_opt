...
VARIANTS = [
    (False, False, "Neither"),
    (False, True,  "No Flat-Top Projection"),
    (True,  False, "No SNR left bound"),
    (True,  True,  "Flat-Top Projection + SNR bound"),
]
...
def main():
    ...
    for _, exp_data in results.items():
        ...
        for idx, (wpp, slb, _) in enumerate(VARIANTS):
    ...
    for (_, _, label), data, offset in zip(VARIANTS, grouped_data, offsets, strict=True):
