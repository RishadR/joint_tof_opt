...
def compute_noise_m1(tof_series: torch.Tensor, bin_edges: torch.Tensor, window: torch.Tensor):
    ...
    variance = (weighted_tof * (bin_centers.unsqueeze(0) - mean_times.unsqueeze(1)) ** 2).sum(dim=1)  # Shape: (num_timepoints,)
...
def compute_noise_variance(tof_series: torch.Tensor, bin_edges: torch.Tensor, window: torch.Tensor):
    ...
    mean_times = (weighted_tof * bin_centers.unsqueeze(0)).sum(dim=1)  # Shape: (num_timepoints,)
    fourth_moment = (weighted_tof * (bin_centers.unsqueeze(0) - mean_times.unsqueeze(1)) ** 4).sum(dim=1)  # Shape: (num_timepoints,)
    variance = (weighted_tof * (bin_centers.unsqueeze(0) - mean_times.unsqueeze(1)) ** 2).sum(dim=1)  # Shape: (num_timepoints,)
    
    noise = (fourth_moment - (variance ** 2) / (N + 1e-20))  # Shape: (num_timepoints,)
    return noise
